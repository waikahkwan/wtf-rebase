...
export class TIMEAVAXLP__burnResult {
  value0: BigInt;
  value1: BigInt;
...
export class TIMEAVAXLP__getReservesResult {
  value0: BigInt;
  value1: BigInt;
  value2: BigInt;
...
export class TIMEAVAXLP extends ethereum.SmartContract {
  static bind(address: Address): TIMEAVAXLP {
    return new TIMEAVAXLP("TIMEAVAXLP", address);
  }
  ...
  burn(to: Address): TIMEAVAXLP__burnResult {
    let result = super.call("burn", "burn(address):(uint256,uint256)", [
      ethereum.Value.fromAddress(to)
    ]);

    return new TIMEAVAXLP__burnResult(
      result[0].toBigInt(),
      result[1].toBigInt()
    );
  }

  try_burn(to: Address): ethereum.CallResult<TIMEAVAXLP__burnResult> {
    ...
    return ethereum.CallResult.fromValue(
      new TIMEAVAXLP__burnResult(value[0].toBigInt(), value[1].toBigInt())
    );
  }
  ...
  getReserves(): TIMEAVAXLP__getReservesResult {
    let result = super.call(
      "getReserves",
    ...
    return new TIMEAVAXLP__getReservesResult(
      result[0].toBigInt(),
      result[1].toBigInt(),
      result[2].toBigInt()
    );
  ...
  try_getReserves(): ethereum.CallResult<TIMEAVAXLP__getReservesResult> {
    let result = super.tryCall(
      "getReserves",
      "getReserves():(uint112,uint112,uint32)",
      []
    ...
    let value = result.value;
    return ethereum.CallResult.fromValue(
      new TIMEAVAXLP__getReservesResult(
        value[0].toBigInt(),
        value[1].toBigInt(),
        value[2].toBigInt()
      )
